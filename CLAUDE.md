...
- **IDs**: prefixed via `createId(kind)` from `@acolhe-animal/shared`; generated in the app, never in the DB.
...
- **Styling**: design tokens in `apps/web/app/globals.css` (Tailwind v4 `@theme`). Colors via utilities (`bg-terra`, `text-ink`, `border-line`) — never hardcoded hex. Primary button is a pill (`rounded-full`).
